...
for(i in 1:length(dists)){
  ...
  ##define treatment group by distance from projects
	...
	N.R = 2
	out.boot = lm.boot(out.reg,
	                   R = N.R)
	
	##sometimes bad draws on the sparse factors cause trouble on estimating
	##standard errors, so put in try functions. Reduce size of simulation for sparse
	##data to point that is needed
...
##only can use complete cases for matching, so extract those, first extract
##needed columns

use.data = white.data[,c('vote.change',
                         'demo.distance',
                         'p',
                         's',
                         'age',
                         'age.squared',
                         'medianincome', 'prior.avg.value','deeded.strict')]
...
for(i in 1:length(dists)){
	...
	outmat.diffs[i,'low.ci'] = ci.boot$normal[2]
	
	##extracting the normal approximation, rather than regular, for 95% confidence
	##interval
	...
	repeat{
		ci.try = try(boot.ci(mean.boot), silent = T)
			...
			mean.boot = two.boot(sample1 = m.data$vote.change[m.data$white==1],
					...
					##need many draws because ci's below require them
...
##bootstrap the standard errors
	...
	##extracting the normal approximation, rather than regular, for 95% confidence interval
